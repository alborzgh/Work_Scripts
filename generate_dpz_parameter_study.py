...
def write_setting(out_dir, num_cases):
    ...
    et.SubElement(values, 'max_thickness').text = str(3)
    et.SubElement(values, 'min_thickness').text = str(0.5)
    et.SubElement(values, 'NumberOfRealizations').text = str(num_cases*6)
    et.SubElement(values, 'Rho0').text = str(0.96)
    et.SubElement(values, 'Rho200').text = str(0.96)
    et.SubElement(values, 'sigma').text = str(0.46)
    et.SubElement(values, 'rho1').text = str(-1)
...
def create_dpz(out_dir, filename, num_cases):
    
    
    num_case = (num_cases)*6    
            
    with ZipFile(os.path.join(out_dir, filename + '.dpz'), 'w') as dpz_file:
        dpz_file.write(os.path.join(out_dir, f'RandomizationSettings'), arcname=f'RandomizationSettings')
...
def create_dpz1(out_dir, filename, num_cases,subcases):   
    
    print(os.path.join(out_dir, filename +subcases+ '.dpz'))
    with ZipFile(os.path.join(out_dir, filename +subcases+ '.dpz'), 'w') as dpz_file:
        dpz_file.write(os.path.join(out_dir, f'RandomizationSettings'), arcname=f'RandomizationSettings')
...
def generate_random_velocities_B(filename, ID_start, num_cases=40, randomize_layers=[1], calibrate_layers=[], out_dir='random_study', norm_dist=[1]):
    ...
    # write files to be zipped
    write_setting(out_dir, num_cases)
    
    if ID_start==2:
        
        # write profile and meta profile 1
        with open(os.path.join(out_dir, f'Profile{ID_start-1}'), 'w') as f:
            f.write("[FILE_VERSION]:[1]\n")
            f.write("[ANALYSIS_DOMAIN]:[TIME+FREQUENCY]\n")
            f.write("[MAX_ITERATIONS]:[15] [COMPLEX_MOD]:[SHAKE_FI] [EFFECTIVE_SSR]:[0.65]\n")
            f.write("[ANALYSIS_TYPE]:[NONLINEAR]\n")
        ...
        mydata = et.ElementTree(doc)
        indent(doc)
        with open(os.path.join(out_dir, f'meta_Profile{ID_start-1}'), 'wb') as f:
            mydata.write(f, encoding='utf-8', xml_declaration=True)
    ...
    for analysis_ii in range(num_cases):
        ...
        mydata = et.ElementTree(doc)
        indent(doc)
        with open(os.path.join(out_dir, f'meta_Profile{analysis_ii+ID_start}'), 'wb') as f:
            mydata.write(f, encoding='utf-8', xml_declaration=True)

    # write profiles
    for analysis_ii in range(num_cases):
        with open(os.path.join(out_dir, f'Profile{analysis_ii+ID_start}'), 'w') as f:
            f.write("[FILE_VERSION]:[1]\n")
            f.write("[ANALYSIS_DOMAIN]:[TIME+FREQUENCY]\n")
            f.write("[MAX_ITERATIONS]:[15] [COMPLEX_MOD]:[SHAKE_FI] [EFFECTIVE_SSR]:[0.65]\n")
            f.write("[ANALYSIS_TYPE]:[NONLINEAR]\n")
            ...
            for layer_ii, layer in enumerate(elements):
                f.write(f"\t[LAYER_NAMES]:[{layer_ii+1}][{layer['Name'].replace(' ','_')}]\n")
    np.savetxt(os.path.join(out_dir, f'Generated_Vs_{ID_start}.csv'), Vs_to_write, delimiter=',')



def generate_random_velocities(filename, num_cases=40, randomize_layers=[1], out_dir='random_study', norm_dist=[1]):
    ...
    for analysis_ii in range(num_cases):
        with open(os.path.join(out_dir, f'Profile{analysis_ii+2}'), 'w') as f:
            ...
            for layer_ii, layer in enumerate(elements):
                if layer_ii in randomize_elements:
                    temp_Vs =  np.exp(np.log(layer['Vs']) + 0.10*norm_dist[layer_ii, analysis_ii])
                    # layer['Vs'] =
                    Vs_to_write[layer_ii, analysis_ii+1] = temp_Vs #layer['Vs']
                f.write(f"[LAYER]:[{layer_ii+1}]\n")
                f.write(f"\t[THICKNESS]:[{layer['thickness']}] [WEIGHT]:[{layer['unit_weight']}] [SHEAR]:[{layer['Vs']}] [SS_DAMP]:[{layer['D']}]\n")
                f.write(f"\t[MODEL]:[GQ] [STRENGTH]:[{layer['strength']}] [THETA1]:[{layer['t1']:.5f}] [THETA2]:[{layer['t2']:.5f}] [THETA3]:[{layer['t3']:.5f}] [THETA4]:[{layer['t4']:.5f}] [THETA5]:[{layer['t5']:.5f}] [A]:[1]\n")
                f.write(f"\t[MRDF]:[UIUC] [P1]:[{layer['p1']:.5f}] [P2]:[{layer['p2']:.5f}] [P3]:[{layer['p3']:.5f}]\n")
...
def main_parameter_study():
    ...
    out_dir = r"C:\Users\FEli\Desktop\PRDP\Bhr-04"
    filename = "Bhr-04"
    
    num_cases = 40
    num_layers_2random = 16
    Gmodels = ['Darendeli','Zhang']
    
    # generate random variables
    norm_dist = np.zeros((num_layers_2random, num_cases))
    for ii in range(num_layers_2random):
        norm_dist[ii,:] = np.random.normal(0.0, 1.0, (1, num_cases))
    
    ## generate dpz file
    ID = 2
    for imodel in Gmodels:
        for jj in range(1,4):
            
            subcase = f"%s_soft%d"% ( imodel,jj)
            xlsx_name = f"%s/%s-%s.xlsx"% (out_dir, filename, subcase)
            
            # generate_random_velocities(filename=xlsx_name, num_cases=num_cases,  out_dir=r"C:\Users\FEli\Desktop\PRDP", norm_dist=norm_dist)
            generate_random_velocities_B(filename=xlsx_name, ID_start=ID, num_cases=num_cases, randomize_layers=[1], calibrate_layers=[], out_dir=out_dir, norm_dist=norm_dist)
            # create_dpz1(out_dir, filename,  num_cases=num_cases, subcases = subcase) ## this function is not fully tested yet
            ID = ID +num_cases
            print(f"%s/%s-%s_soft%d.xlsx"% (out_dir, filename, imodel,jj))

    create_dpz(out_dir, filename, num_cases)
